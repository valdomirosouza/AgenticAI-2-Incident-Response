...
| **Embedding**              | Representação numérica (vetor de alta dimensão) de um texto gerada por um modelo de ML, que preserva relações semânticas: textos com significado próximo resultam em vetores próximos no espaço vetorial.                                                                                                                                                                               |
| **Vector Database**        | Banco de dados especializado no armazenamento e recuperação de embeddings. Permite busca por similaridade semântica (ex.: "quais incidentes passados se parecem com este?") em vez de busca por palavras-chave exata. No projeto: **Qdrant v1.18**, coleção `incidents`, distância cosseno, 384 dimensões.                                                                              |
| **Semantic Search**        | Busca por similaridade de significado entre vetores, ao contrário da busca léxica por palavras-chave. O orchestrator usa semantic search para recuperar chunks históricos relevantes a partir dos summaries dos findings não-OK dos especialistas.                                                                                                                                      |
| **Chunk / Chunking**       | Segmentação de um documento (ex.: post-mortem) em unidades menores e semanticamente coesas para armazenamento vetorial. Cada chunk recebe um tipo (`symptom_fingerprint`, `runbook_step`, `lesson_learned`, etc.) e metadados estruturados que permitem filtragem híbrida (semântica + metadata).                                                                                       |
| **RAG**                    | Retrieval-Augmented Generation — técnica que combina recuperação de documentos relevantes (via Vector Database) com geração de texto (via LLM). No projeto: o orchestrator recupera chunks históricos da KB e os injeta no prompt de síntese do Claude para fundamentar o diagnóstico em casos reais.                                                                                   |
| **KB / Knowledge Base**    | Base de conhecimento vetorial que armazena incidentes históricos como chunks tipados (98 chunks dos incidentes INC-001 a INC-004). Exposta pelo módulo `Knowledge-Base` (porta 8002). Consultada automaticamente pelo orchestrator quando há findings não-OK, enriquecendo o diagnóstico com padrões de sintomas, runbooks e lições aprendidas de incidentes anteriores.                |
| **Seed / Seeding (KB)**    | Processo de pré-carga da Knowledge Base com incidentes históricos. Executado via `scripts/seed_kb.py`, que converte cada post-mortem em chunks tipados, embeda-os e os armazena no Qdrant. Deve ser executado uma vez após o stack subir.                                                                                                                                               |
| **Runbook / Playbook**     | Procedimentos padronizados e fluxos de decisão para guiar a resposta a incidentes.                                                                                                                                                                                                                                                                                                      |
| **Post-mortem**            | Documento estruturado produzido após a resolução de um incidente, registrando: causa raiz (blameless), linha do tempo, impacto, ações executadas, lições aprendidas e itens de follow-up. No projeto: armazenados em `docs/post-mortems/` e usados como fonte primária para o seed da KB. Cada post-mortem é decomposto em chunks tipados para permitir recuperação semântica granular. |
...
| **similar_incidents**      | Campo do `IncidentReport` que lista os IDs dos incidentes históricos recuperados da KB cujos padrões de sintoma mais se aproximaram do incidente atual (ex.: `["INC-001", "INC-003"]`). Preenchido pelo orchestrator após a etapa de RAG; vazio quando todos os findings são OK e a KB não é consultada.                                                                                |
| **Graceful Degradation**   | Comportamento em que um componente falha silenciosamente sem propagar o erro para o fluxo principal. No projeto: `kb_client.search_kb()` captura qualquer exceção e retorna `[]` — a KB indisponível nunca bloqueia a análise.                                                                                                                                                          |
